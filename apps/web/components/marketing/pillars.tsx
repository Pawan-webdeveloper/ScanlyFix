...
export function Pillars() {
  ...
  return (
    <Section id="checks">
      ...
      {/* The grid is genuinely a list of six, so its cards cascade in once
          (home-motion.tsx); the counts count up — they are the claim. */}
      <ul data-motion="stagger" className="mt-12 grid gap-px overflow-hidden border border-line bg-line sm:grid-cols-2 lg:grid-cols-3">
        {pillars.map((pillar) => (
          <li key={pillar.category} className="relative min-w-0 overflow-hidden bg-canvas">
            ...
            <div className="relative flex min-w-0 flex-col p-7">
              <div className="flex items-baseline justify-between gap-3">
                ...
                <span data-motion-count className="font-mono text-lg font-semibold text-ink tabular-nums">{pillar.count}</span>
              </div>

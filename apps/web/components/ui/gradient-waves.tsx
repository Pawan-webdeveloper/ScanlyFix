...
/**
 * GradientWaves — a WebGL wave field drawn behind the pillar cards, from
 * React Bits (reactbits.dev), ported to TypeScript. The container class of
 * the original became Tailwind utilities; note the base classes deliberately
 * do NOT include `position` — callers place the canvas (`absolute inset-0`
 * for a card background, or inside an explicitly sized relative wrapper).
 *
 * The render loop is disciplined about resources: the raymarch pauses when
 * the canvas scrolls out of view or the tab hides, the device pixel ratio is
 * capped at 2, and unmounting loses the GL context explicitly — four cards
 * each own a context, and an abandoned one is a leaked one.
 */
...
/** One GL pipeline per container, shared across prop updates via the WeakMap. */
...
const GradientWaves = ({
  ...
  useEffect(() => {
    ...
    // The pipeline is built once; the effect below keeps it in step with props.
